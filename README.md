...
around the panner sound source (orange circle).
...
Показан оранжевым кружком со стрелкой, которая обозначает направление распространения звука.
...
Светло-оранжевый круг вокруг источника обозначает область, ограниченную расстоянием `refDistance`.

Темно-оранжевый круг вокруг источника обозначает границу `maxDistance` (появляется только при выборе значения `linear` параметра `distanceModel`).
